<template>
  <div class="adminFilterStores">
    <DiststoretoStore
      v-if="openStoreToStoreDist"
      @regetData="regetData"
      @close_popup="openStoreToStoreDist = false"
      :openPopup="openStoreToStoreDist"
      :fromStore="fromStore"
      :distPart="distPart"
      :fromStoreName="singleStore.content[0].store.storeName"
      :storeId="singleStore.content[0].store.id"
    />
    <v-container class="mt-4" fluid>
      <div
        class="table_parent bg-white mb-0"
        :style="`height: ${windowHeight - 225}px; overflow: scroll`"
      >
        <table
          class="w-100 text-center"
          v-if="!tableLoading && singleStore && singleStore.content.length"
        >
          <caption
            style="
              background-color: rgb(114, 49, 116);
              color: white;
              margin: 0 2px;
              font-weight: bold;
              font-size: 16px;
            "
            class="py-3"
          >
            {{
              singleStore.content[0].store.storeName
            }}
          </caption>
          <thead>
            <tr>
              <th v-for="head in tableHeaders" :key="head">{{ head.text }}</th>
              <th v-if="loggerData.userPrivileges.haveStorePartsDist">
                اتخاذ اجراء
              </th>
            </tr>
          </thead>
          <tbody style="font-size: 15px">
            <tr v-for="(list, i) in singleStore.content" :key="list.id">
              <td style="width: 12%">
                {{ list.part.partName }}
              </td>
              <td style="width: 12%">
                {{
                  list.amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
                }}
              </td>
              <td style="width: 12%">
                <img
                  :src="`data:image/jpg;base64,${list.part.picture}`"
                  alt=""
                  width="30"
                  height="30"
                  style="border-radius: 50%"
                />
              </td>
              <td
                style="width: 5%"
                v-if="loggerData.userPrivileges.haveStorePartsDist"
              >
                <v-btn variant="text" :disabled="list.amount <= 0">
                  <v-icon
                    :id="`actionsMenu_${i}`"
                    color="blue"
                    style="cursor: pointer"
                    >mdi-dots-vertical</v-icon
                  >
                </v-btn>
                <v-menu :activator="`#actionsMenu_${i}`" offset="15">
                  <v-list class="pa-2" elevation="3">
                    <v-list-item @click="startDist(list)">
                      <v-list-item-title>
                        <v-icon>mdi-store</v-icon>
                        <v-icon size="13">mdi-arrow-right</v-icon>
                        توزيع الي مخزن
                      </v-list-item-title>
                    </v-list-item>
                  </v-list>
                </v-menu>
              </td>
            </tr>
          </tbody>
        </table>
        <div
          v-if="
            (!singleStore && !tableLoading) ||
            (singleStore && !singleStore.content.length && !tableLoading)
          "
          :style="`min-height: ${windowHeight - 250}px`"
          class="no_data"
        >
          <p>لا توجد نتائج</p>
        </div>
        <v-progress-linear
          style="width: 100%"
          indeterminate
          color="rgb(60 22 68)"
          v-if="tableLoading"
        ></v-progress-linear>
      </div>
      <v-divider color="black"></v-divider>
      <v-row class="pt-4 align-center bg-white">
        <v-col cols="3" class="pt-2">
          <v-select
            :items="ranges"
            v-model="size"
            variant="outlined"
            label="عدد النتائج"
            @update:model-value="selectSize"
            class="mr-3"
            style="transform: translateY(8px); width: 100px"
            v-if="singleStore && singleStore.content.length"
            :disabled="tableLoading"
          ></v-select>
        </v-col>
        <v-col cols="6" class="d-flex justify-center">
          <v-pagination
            v-model="page"
            :length="singleStore ? singleStore.totalPages : 2"
            :total-visible="singleStore.content ? singleStore.size : 5"
            prev-icon="mdi-chevron-right"
            next-icon="mdi-chevron-left"
            @update:model-value="doGetSearch"
            v-if="singleStore && singleStore.content.length"
            :disabled="tableLoading"
          >
          </v-pagination>
        </v-col>
      </v-row>
    </v-container>
    <v-menu activator="#stores-activator">
      <v-list elevation="3">
        <v-list-item
          prepend-icon="mdi-printer"
          @click="printData"
          v-if="singleStore && singleStore.content.length"
        >
          <v-list-item-title>طباعة الجدول</v-list-item-title>
        </v-list-item>
        <v-list-item
          prepend-icon="mdi-download"
          @click="exportExcel"
          v-if="singleStore && singleStore.content.length"
        >
          <v-list-item-title>تحميل الجدول</v-list-item-title>
        </v-list-item>
        <v-list-item
          prepend-icon="mdi-send"
          @click="settleStore = true"
          v-if="
            singleStore &&
            singleStore.content.length &&
            loggerData.userPrivileges.settlementStore
          "
        >
          <v-list-item-title>تسوية المخزن</v-list-item-title>
        </v-list-item>
        <v-list-item prepend-icon="mdi-magnify" @click="openSearch = true">
          <v-list-item-title style="margin-top: -5px">بحث</v-list-item-title>
        </v-list-item>
      </v-list>
    </v-menu>
    <v-icon
      id="stores-activator"
      style="
        position: fixed;
        left: 20px;
        bottom: 40px;
        z-index: 2;
        border-radius: 50%;
        padding: 30px;
        box-shadow: 0px 0px 15px -2px black;
        cursor: pointer;
        background: rgb(69, 22, 70);
        color: white;
      "
      >mdi-menu</v-icon
    >
    <AdminstoreSearch
      v-if="openSearch"
      :openSearch="openSearch"
      @closePopup="openSearch = false"
    />
    <StoreSettlement
      v-if="settleStore"
      :openPopup="settleStore"
      @closePopup="settleStore = false"
      :storeId="storeId"
      :storeName="singleStore.content[0].store.storeName"
      @doGetSearchedLists="regetData"
    />
  </div>
</template>

<script setup>
import { ref, defineProps, onMounted, watch } from "vue";
import { storesModule } from "@/stores/the_stores/theStores";
import { mainStore } from "@/stores";
import { authStore } from "@/stores/auth/auth";
import DiststoretoStore from "@/components/store_to_store/DiststoretoStore.vue";
import { storeToRefs } from "pinia";
import { useRoute } from "vue-router";
import AdminstoreSearch from "@/components/entities/stores/AdminstoreSearch.vue";
import StoreSettlement from "@/components/store_to_store/StoreSettlement.vue";

// Init store
const storesMod = storesModule();
const mainSt = mainStore();
const authModule = authStore();

// Init route
const route = useRoute();

// Data
const { singleStore } = storeToRefs(storesMod);
const { loggerData } = storeToRefs(authModule);
const page = ref(1);
const size = ref(7);
const ranges = [7, 14, 21, 35];
const tableLoading = ref(false);
const storeId = ref("");
const openStoreToStoreDist = ref(false);
const fromStore = ref("");
const distPart = ref("");
const openSearch = ref(false);
const settleStore = ref(false);

const tableHeaders = ref([
  {
    text: "اسم الجزء",
    value: "id",
  },
  {
    text: "الكمية المتاحة",
    value: "supplier.supplierName",
  },
  {
    text: "صورة الجزء",
    value: "creationDate",
  },
]);

// Watchers
watch(
  () => route,
  () => {
    if (route.name == "adminStores") {
      page.value = 1;
      size.value = 7;
      routeValus();
      doGetSearch();
    }
  },
  { deep: true }
);

// Props
defineProps(["windowHeight"]);

// Methods
const routeValus = () => {
  storeId.value = route.query.storeId ? route.query.storeId : "";
};

const doGetSearch = async () => {
  if (!storeId.value) {
    singleStore.value = "";
    return;
  }
  const doViewStore = storesMod.doViewStore;
  const routeObj = {
    storeId: storeId.value,
    page: page.value,
    size: size.value,
  };
  tableLoading.value = true;
  await doViewStore(routeObj);
  tableLoading.value = false;
};

const selectSize = async () => {
  page.value = 1;
  await doGetSearch();
};

const regetData = () => {
  doGetSearch();
};

const startDist = (data) => {
  fromStore.value = data.store.id;
  distPart.value = data;
  openStoreToStoreDist.value = true;
};

const exportExcel = () => {
  let lists = JSON.parse(JSON.stringify(singleStore.value.content));
  let arr = [];
  lists.forEach((list) => {
    let obj = {
      "اسم الجزء": list.part.partName,
      "الكمية المتاحة": list.amount,
    };
    arr.push(obj);
  });
  console.log(arr);
  var wscols = [
    { wch: 20 },
    { wch: 20 },
    { wch: 20 },
    { wch: 20 },
    { wch: 20 },
  ];
  mainSt.exportExcel(arr, `تفاصيل مخزن ${lists[0].store.storeName}`, wscols);
};

const printData = () => {
  let data = JSON.parse(JSON.stringify(singleStore.value.content));
  let arr = [];
  let props = ["اسم الجزء", "الكمية المتاحة"];
  data.forEach((list) => {
    let obj = {
      "اسم الجزء": list.part.partName,
      "الكمية المتاحة": list.amount,
    };
    arr.push(obj);
  });
  mainSt.printTable(arr, props, `تفاصيل مخزن ${data[0].store.storeName}`);
};
// Hooks
onMounted(async () => {
  routeValus();
  await doGetSearch();
  console.log(loggerData.value.userPrivileges);
});
</script>

<style lang="scss">
.adminFilterStores {
  table {
    overflow-y: scroll;
    background-color: white;
    thead {
      font-size: 14px;
      tr {
        box-shadow: 0px 0px 3px -2px black;
      }
    }
    tbody {
      tr {
        box-shadow: 0px 0px 3px -2px black;
        th {
          padding: 10px 0 !important;
        }
        td {
          padding: 0 !important;
        }
      }
    }
  }
  .v-pagination__list {
    justify-content: flex-start;
  }
  .pagins_parent {
    padding: 0 10px 0 35px;
  }
  .no_data {
    display: flex;
    align-items: center;
    justify-content: center;
    p {
      font-size: 25px;
      color: rgb(80, 80, 80);
    }
  }
  .no_data {
    display: flex;
    align-items: center;
    justify-content: center;
    p {
      font-size: 25px;
      color: rgb(80, 80, 80);
    }
  }
}
.v-list-item__prepend > .v-icon {
  margin-inline-end: 10px !important;
}
</style>
