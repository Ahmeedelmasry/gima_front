    ...
    <v-container class="mt-4" fluid>
      ...
      >
        ...
        >
          ...
          >
            {{
              requests.content[0].storeTo.storeName
            }}
          </caption>
          <thead>
            <tr>
...
watch(
  () => route,
  async () => {
    if (route.name == "AdminToStoreReqs") {
      page.value = 1;
      size.value = 7;
      routeValus();
      await doGetSearch();
...
const exportExcel = () => {
  ...
  reqs.forEach((req) => {
    let obj = {
      ...
      "من مخزن": req.storeFrom.storeName,
      "حالة الطلب":
  ...
  mainSt.exportExcel(
    arr,
    `تقارير توزيع مخزني الي ${reqs[0].storeTo.storeName}`,
    wscols
  );
};
...
const printData = () => {
  ...
  mainSt.printTable(
    arr,
    props,
    `تقارير توزيع مخزني الي ${data[0].storeTo.storeName}`
  );
};
