...
// Data
const { supplyRejected } = storeToRefs(storeReqs);
...
const exportExcel = () => {
  let data = JSON.parse(JSON.stringify(supplyRejected.value.content));
  console.log(data);
  let arr = [];
  data.forEach((list) => {
    let obj = {
  ...
  mainSt.exportExcel(arr, "قائمة طلبات توزيع التوريدات المرفوضة", wscols);
};
...
const printData = () => {
  ...
  mainSt.printTable(arr, props, "قائمة طلبات توزيع التوريدات المرفوضة");
};
