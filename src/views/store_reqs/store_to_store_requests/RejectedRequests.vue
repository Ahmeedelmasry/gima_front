...
// Data
const { storeRejected } = storeToRefs(storeReqs);
...
const exportExcel = () => {
  let data = JSON.parse(JSON.stringify(storeRejected.value.content));
  console.log(data);
  let arr = [];
  data.forEach((list) => {
    let obj = {
  ...
  mainSt.exportExcel(arr, "قائمة طلبات توزيع مخزني مرفوضة", wscols);
};
...
const printData = () => {
  ...
  mainSt.printTable(arr, props, "قائمة طلبات توزيع مخزني مرفوضة");
};
